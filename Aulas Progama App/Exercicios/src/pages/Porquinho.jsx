...
function Porquinho() {
    ...
    const [input05cent, setInput05cent] = useState('')
    const [sobrou, setSobrou] = useState(false)
    const [faltou, setfaltou] = useState(false)

    function quebrarProco(){
        let dinheiro = Number((input1real *1) + (input50cent *0.5) + (input25cent * 0.25) + (input10cent * 0.1) + (input05cent * 0.05))
  ...
  return (
    ...
    <div className='div-porquinho-container'>
      ...
      <div className="form-container">
          ...
          <div className="input-container">
            ...
            value={input05cent}
            onChange={(event) => setInput05cent(event.target.value)}
            />
          </div>
      </div>
      <button onClick={quebrarProco}>QuebrarPorco</button>
      {sobrou && <TemDinheiro />}
